...
export const MARKDOWN_COMPONENTS = {
  ...
  thead: ({ children, className, ...props }: MarkdownComponentProps) => (
    <thead className={cn('bg-primary/5', className)} {...props}>
      {children}
    </thead>
  ),
  tr: ({ children, className, ...props }: MarkdownComponentProps) => (
    <tr className={cn('border-b border-border/50 last:border-b-0 even:bg-muted/20 hover:bg-muted/30 transition-colors', className)} {...props}>
      {children}
    </tr>
  ),
  th: ({ children, className, ...props }: MarkdownComponentProps) => (
    <th className={cn('px-3 py-2.5 text-left font-semibold text-xs whitespace-nowrap text-primary', className)} {...props}>
      {children}
    </th>
  ),
  td: ({ children, className, ...props }: MarkdownComponentProps) => (
    <td className={cn('px-3 py-2 text-sm tabular-nums', className)} {...props}>
      {children}
    </td>
  ),
